import { config, defOpts } from '$lib/reveal/config';
import { hasValidBreakpoints } from '$lib/reveal/styling';
import { createFinalOptions, createObserverConfig, getConfigClone } from '$lib/reveal/utils';
import { areOptionsValid, hasValidRange } from '$lib/reveal/validation';
import type { RevealConfig, Device, IDevice, Responsive, RevealOptions, IObserverOptions } from '$lib/reveal/types';
import { ROOT_MARGIN_REGEX } from '$lib/reveal/constants';
...
export const defaultFullBox = (options: RevealOptions): Required<RevealOptions> => {
	const validOptions = createFinalOptions(options)
	if (!areOptionsValid(validOptions)) {
		showChip('Invalid Options!', '#fe4a49')
	}
