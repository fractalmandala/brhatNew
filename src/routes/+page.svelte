	...
	<div class="rta-column ycenter rowgap600 outer-box minH" data-lenis-scroll-snap-align="start">
			...
			<div class="rta-row row-col ycenter between rowgap100 colgap400 glass-top p-top-32 bord-bot p-bot-32">
				...
				<div class="colgap100 rta-row selfend xend ycenter">
					<div class="arrowsanimation">
						<svg width="94" height="24" viewBox="0 0 94 32" fill="none" xmlns="http://www.w3.org/2000/svg">
							<g clip-path="url(#clip0_15_2638)">
								<path id="arrowl" d="M94.0712 4.6096L90.4616 1L75.1016 16.36L90.4616 31.72L94.0712 28.1104L82.3464 16.36L94.0712 4.6096Z" fill="#FE4A49"/>
								<path id="arrowc" d="M64.2839 5.29L61.1606 2L47.8701 16L61.1606 30L64.2839 26.71L54.1388 16L64.2839 5.29Z" fill="#FE4A49"/>
								<path id="arrowr" d="M35.9675 8.4675L33.5 6L23 16.5L33.5 27L35.9675 24.5325L27.9525 16.5L35.9675 8.4675Z" fill="#FE4A49"/>
							</g>
							<defs>
		...
		<div class="rta-column rowgap300" use:autoAnimate>
			{#if chapters && chapters.length > 0}
				...
				<div class="rta-row row-col ycenter rta-drawer-items colgap400 rowgap100 p-bot-16" use:autoAnimate on:click={toggleMenu} on:keydown={fauxfake}>
					{#each chapters as item, i}
						<button class="drawer-item" on:click={() => toggleAnveshi(i)} on:keydown={fauxfake} class:drawerselection={anveshiFull[i]}>{item.name}</button>
					{/each}
				</div>
				{/if}
				<div class="rta-column" use:autoAnimate> 
					{#each chapters as item, i}
						...
						<div class="rta-row row-col colgap400 rowgap200">
							...
							<div class="rta-column w32 rowgap200">
	...
	<div class="rta-column rowgap600 ycenter outer-box minH" data-lenis-scroll-snap-align="start">
		...
		<div class="rta-grid left grid2 rowgap400 colgap500" use:autoAnimate>
			{#if courses && courses.length > 0}
				...
				<div class="rta-column rta-drawer-items rowgap200 bord-right-d" id="drashta-drawer" on:click={toggleMenu2} on:keydown={fauxfake} use:autoAnimate>
					{#each courses as item, i}
						<button class="drawer-item2" on:click={() => toggleDrashta(i)} on:keydown={fauxfake} class:drawerselection={drashtaFull[i]}>{item.coursename}</button>
					{/each}
				</div>
				{/if}
				{#each courses as item, i}
					{#if drashtaFull[i]}
					{#if item.status === 'upcoming' || item.status === 'Upcoming'}
						<div id="drashtascreen" class="rta-column height-40 p-all-32 back" style="background-image: url('{item.image}')" use:autoAnimate>
								<div class="rta-column rowgap100 w32">
									<div class="status-sticker">
										<small class="tt-u">{item.status}</small>
									</div>
									<cite class="sticker-black">{item.datefrom} | with <span id="instructor">{item.ins}</span></cite>
								</div>
						</div>
						{:else}
