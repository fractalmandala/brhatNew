...
<section class="sliderbox">
	<div class="sliver carries" style="background-image: url('/images/newpages/members.webp')" />
	<div class="title rta-column rowgap100 xright">
		<h3>Members Area</h3>
		{#if isAuth}
			<form method="post" action="?/signout" use:enhance={handleSignOut} class="rta-column xright">
				<button class="buttonmaingrain" disabled={loading}>Sign Out</button>
			</form>
		{:else if !isAuth}
			<form method="post" action="?/login">
				<div class="rta-column xright rowgap100 null">
   ...
   <!--
       <button
    class="blank-button rta-row xend ycenter colgap100 google"
    on:click={() => {
    supabase.auth.signInWithOAuth({
     provider: 'google',
     options: { scopes: 'https://www.googleapis.com/auth/userinfo.email', redirectTo: $page.url.pathname }
     });
    }}
   >
    <p class="small">Sign In with Google</p>
    <img class="iconimage" src="/images/icongoogle.png" alt="google" />
   </button>
   -->
		{/if}
	</div>
	<div class="image carries" style="background-image: url('/images/newpages/members.webp')">
  ...
  <form method="post" action="?signup">
   <div class="rta-column xleft rowgap100 null">
    ...
    <button disabled={loading} class="buttonmain">Sign Up</button>
   </div>
  </form>
  {/if}
	</div>
