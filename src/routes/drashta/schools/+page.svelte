...
<svelte:head>
	...
	<div class="rta-column outer-box rowgap600 limit">
		...
		<div class="rta-grid grid4 colgap400 rowgap400 p-bot-64" class:calibrated={anyDrashtaOpen} use:autoAnimate>
		{#if drashtas && drashtas.length > 0}
			{#each drashtas as item, i}
				{#if selectedDrashta[i]}
					<div class="rta-column rowgap200 opentab point" on:click={() => toggleGenre(i)} on:keydown={() => toggleGenre(i)}>
